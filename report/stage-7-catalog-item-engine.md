...
| File | Purpose |
|------|---------|
| `src/CatalogItem.php` | Pure data object holding normalized catalog item data (id, type, title, SKU, price, stock status, image, permalink, categories, tags, context) |
| `src/CatalogContext.php` | Minimal context value object (catalog_id, language, currency) |
| `src/CatalogItemMapper.php` | Maps WC_Product → CatalogItem, handles simple/variable/variation types |
| `tests/Unit/CatalogItemTest.php` | Unit tests for CatalogItem and CatalogContext (pure PHP, no WP/WC) |
| `tests/Integration/CatalogItemMapperTest.php` | Integration tests for mapping WC_Product → CatalogItem |

### Design Decisions

- **CatalogItem** is a plain data object with public typed properties and type helper methods (`is_simple()`, `is_variation()`, `is_variable()`). No behavior beyond data access.
